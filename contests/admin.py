...
class ACMContestAdmin(admin.ModelAdmin):
    ...
    'show_final_ranking_onsite' ,'show_final_ranking_online', 'show_test_data','show_judge_solution' )
    # list_filter = ('title',)
    search_fields = [ 'title']

    def show_problem(self, obj):
        return format_html("<a href='{url}'>{text}</a>", url=obj.problems , text="problems"+obj.title)
    show_problem.short_description = "problems"
    
    def show_final_ranking_onsite(self, obj):
        return format_html("<a href='{url}'>{text}</a>", url="//www.google.com", text="final_ranking_onsite")
    show_final_ranking_onsite.short_description = "final_ranking_onsite"
   
    def show_final_ranking_online(self, obj):
        return format_html("<a href='{url}'>{text}</a>", url="//www.google.com", text="final_ranking_online")
    show_final_ranking_online.short_description = "final_ranking_online"
